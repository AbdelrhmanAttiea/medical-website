    ...
    <div className="h-screen flex flex-col">
      <div className="bg-gray-200 flex-1 overflow-y-scroll">
        ...
        {messages.map((message, index) => (
          <div key={index} className={`px-4 py-2 ${message.sender === auth().currentUser.uid ? 'justify-end' : 'justify-start'}`}>
            <div className="flex items-center mb-2">
              {message.sender !== auth().currentUser.uid && <img className="w-8 h-8 rounded-full mr-2" src={message.senderAvatar} alt="User Avatar" />}
              <div className={`font-medium ${message.sender === auth().currentUser.uid ? 'text-right' : ''}`}>{message.senderName}</div>
            </div>
            <div className={`bg-white rounded-lg p-2 shadow mb-2 max-w-sm ${message.sender === auth().currentUser.uid ? 'bg-blue-500 text-white' : ''}`}>
              {message.message}
            </div>
          </div>
        ))}
